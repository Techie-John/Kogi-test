...
class User(AbstractUser):
    ...
    profile_photo = models.ImageField(default='default.png', upload_to='profile_pics', blank=True)
    bio = models.TextField(max_length=100)

class Artist(models.Model):
    artistName = models.CharField(_("Artist Name"), max_length=80)
    profile_photo = models.ImageField(_("Profile Photo"),upload_to='thumbnail/', help_text=".jpg, .png, .jpeg, .gif, .svg supported")
    ...
    biography = models.CharField(_("Biography"), max_length=250)
...
class PromotedSong(models.Model):
    album = models.ForeignKey("Album", verbose_name=_("Song Album"), on_delete=models.CASCADE)
    songThumbnail = models.ImageField(_("Song Thumbnail"),upload_to='thumbnail/', help_text=".jpg, .png, .jpeg, .gif, .svg supported")
    song = models.FileField(_("Promoted Songs"), upload_to='songs/', help_text=".mp3 supported only",)
    songName = models.CharField(_("Promoted Song Name"), max_length=50)
    created = models.DateTimeField(_("Song created date"), auto_now_add=True)
    last_updated = models.DateTimeField(_("Latest song update"), auto_now=True)

    class Meta:
        verbose_name = _("Promoted Song")
        verbose_name_plural = _("Promoted Songs")
...
class RequestedSong(models.Model):
    ...
    year_released = models.DateTimeField(auto_now_add=False)
